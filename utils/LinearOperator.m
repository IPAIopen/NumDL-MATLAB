...
%
% Allows one to define a matrix by its size and acting on a vector. This is
% a basic implementation that overloads  *, transpose, +, blkdiag, hcat, ..
classdef LinearOperator 
    
    properties
        m    % number of columns
        n    % number of rows
        Amv  % mat-vec, function handle
        ATmv % transpose mat-vec, function handle
    end
    
    methods
        function this = LinearOperator(varargin)
            % constructor, LinearOperator(varargin)
            %
            % As input, either provide a matric or define m,n,Amv,ATmv
            if nargin==0
